...
async fn provider_onboarding_filters_models_before_reasoning_selection() {
    ...
    // A pending ordinary picker refresh must not replace the provider-scoped onboarding view.
...
async fn model_picker_queued_all_models_uses_refreshed_catalog() {
    ...
    let expected_first_model = refreshed[0].model.clone();
    ...
    chat.open_all_models_popup();
    assert_chatwidget_snapshot!(
        "model_picker_queued_all_models_uses_refreshed_catalog",
        render_bottom_popup(&chat, /*width*/ 80)
    );
    // The fork's searchable picker starts at the first result, rather than the current model.
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
    let selected =
        assert_matches!(rx.try_recv(), Ok(AppEvent::OpenReasoningPopup { model }) => model.model);
    assert_eq!(selected, expected_first_model);
}
