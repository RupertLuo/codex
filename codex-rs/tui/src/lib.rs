...
mod model_catalog;
mod model_migration;
#[path = "catalyst/model_runtime.rs"]
mod model_runtime;
mod motion;
mod multi_agents;
...
async fn start_embedded_app_server(
    ...
    let thread_manager_options = runtime_options.thread_manager;
    start_embedded_app_server_with(
        ...
        environment_manager,
        move |args| {
            InProcessAppServerClient::start_with_thread_manager_options(
                args,
                thread_manager_options,
            )
        },
    )
    .await
...
mod tests {
    ...
    #[test]
    fn process_local_runtime_options_disable_daemon_reuse_and_remote_targets() {
        let runtime_options = TuiRuntimeOptions::default().with_thread_manager_options(
            ThreadManagerRuntimeOptions::default()
                .with_required_base_instructions("host instructions".to_string()),
        );
        ...
        assert_eq!(
            validate_runtime_target(&runtime_options, /*has_explicit_remote_endpoint*/ true)
                .expect_err("process-local runtime must reject a remote app-server")
                .kind(),
            std::io::ErrorKind::InvalidInput,
        );
    }
