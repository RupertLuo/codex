...
impl<'call> ToolExecutor<ExtensionToolCall<'call>> for StandaloneSearchExtensionTool {
    fn tool_name(&self) -> ToolName {
        self.0.tool_name()
    }
...
async fn hosted_web_search_and_standalone_image_generation_follow_runtime_gates() {
    ...
    let host_managed_image_generation = probe_with(
        |turn| {
            use_bedrock_provider(turn);
            update_turn_settings_for_test(turn, |settings| {
                Arc::make_mut(&mut settings.model_info).input_modalities = vec![];
    ...
    .await;
    host_managed_image_generation.assert_visible_contains(&["catalyst_image"]);
