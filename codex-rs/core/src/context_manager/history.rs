...
impl ContextManager {
    ...
    ///
    /// Images can exhaust a provider's request-body limit while the history remains far inside
    /// the token window. Oversized images are counted at their expected post-reencoding size so
    /// this estimate does not compact a thread merely because its stored PNG is large.
        ...
        fn content_bytes(items: &[ContentItem]) -> u64 {
            items
                ...
                .map(|item| match item {
                    ...
                    ContentItem::InputAudio { audio_url } => audio_url.len() as u64,
