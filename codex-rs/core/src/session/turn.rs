...
async fn run_pre_sampling_compact(
    ...
    let token_status =
        super::context_window::context_window_token_status(sess.as_ref(), turn_context.as_ref())
            .await;
    // Images can exhaust a provider's request-body limit while the history remains comfortably
    // inside the token window, so embedding hosts may also request pre-turn compaction by bytes.
...
async fn over_request_body_budget(sess: &Session, turn_context: &TurnContext) -> bool {
    ...
    let Some(limit) = sess
        .services
        .model_client
        .runtime_policy(&turn_context.model_info.slug)
