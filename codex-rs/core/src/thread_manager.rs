...
impl ThreadManager {
    ...
    pub async fn spawn_native_agent(
        ...
        let initial_input = match request.initial_operation {
            Op::UserInput { items, .. } => items,
            _ => {
                return Err(CodexErr::InvalidRequest(
                    "native agent initial operation must be user input".to_string(),
                ));
            }
        };
        let spawned = parent
            ...
            .services
            .agent_control
            .spawn_agent_with_metadata(request.config, initial_input, Some(session_source), options)
            .await?;
        Ok(NativeAgentSpawn {
