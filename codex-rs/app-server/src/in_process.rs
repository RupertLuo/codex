...
pub async fn start(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    start_with_thread_manager_options(args, ThreadManagerRuntimeOptions::default()).await
}

/// Starts an embedded app-server with process-local thread runtime overrides.
pub async fn start_with_thread_manager_options(
    mut args: InProcessStartArgs,
    thread_manager_runtime_options: ThreadManagerRuntimeOptions,
    ...
    let client = start_uninitialized(args, thread_manager_runtime_options).await?;
            ...
            tokio::select! {
                ...
                queued_message = writer_rx.recv() => {
                    ...
                    match outgoing_message {
                        ...
                        OutgoingMessage::RawResponse(response) => {
                            if let Some(response_tx) = pending_request_responses.remove(&response.id) {
                                let _ = response_tx.send(Ok(response.result));
                            } else {
                                warn!(
                                    request_id = ?response.id,
                                    "dropping unmatched in-process extension response"
                                );
                            }
                        }
