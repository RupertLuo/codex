...
fn has_retryable_tls_error(error: &(dyn Error + 'static)) -> bool {
    ...
    while let Some(error) = source {
        ...
        // Some Linux TLS wrappers expose rustls' enum-style `ProtocolVersion` without spaces.
        let is_enum_protocol_version_error = message.contains("protocolversion");
        ...
        if is_macos_protocol_version_error
            ...
            || is_enum_protocol_version_error
